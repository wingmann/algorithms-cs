...
{
    public void Sort<T>(ISequentialStorage<T> mainMemory, ISequentialStorage<T> temporaryMemory, IComparer<T> comparer)
    {
        ...
        if (source == mainMemory)
        {
            return;
        }

        using var sorted = source.GetReader();
        using var dest = mainMemory.GetWriter();
    ...
    private static void Merge<T>(ISequentialStorageReader<T> left, ISequentialStorageReader<T> right,
    ...
    private static void Copy<T>(ISequentialStorageReader<T> from, ISequentialStorageWriter<T> to, long count)
    {
